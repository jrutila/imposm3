...
# Build binary
#RUN cd /go/src/cmd/imposm \
 	#&& go build -v -ldflags "-w -X 'cmd/imposm/cmd.Version=${VERSION}'" -gcflags "-N -l" -o /opt/imposm3 \
	#&& chmod a+x /opt/imposm3
ENV GOTAGS="-o /opt/imposm3"
RUN cd /go/src/ \
	&& make build 

RUN chmod a+x /opt/imposm3
...
COPY --from=build /opt/imposm3 /opt/
WORKDIR /opt
ENTRYPOINT ["/opt/imposm3"]
